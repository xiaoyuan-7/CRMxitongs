...
old_js = '''function loadFusionData() {
        ...
        function scrollToManager(targetType) {
            var el = document.getElementByById('fusion_type_' + targetType);
...
if old_js in content:
    ...
    print("Old JS block not found - searching for signature...")
    idx = content.find('function loadFusionData')
    idx2 = content.find('function addFusionCompany')
    print(f"loadFusionData at: {idx}, addFusionCompany at: {idx2}")
    # Try partial match
    if 'function addFusionFollow' in content:
        print("addFusionFollow found")
    if 'function editFusionFollow' in content:
        print("editFusionFollow found")
