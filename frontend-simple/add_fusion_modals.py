#!/usr/bin/env python3
"""Replace prompt() calls with proper modal dialogs in fusion module"""

with open('/home/admin/.openclaw/workspace/crm-system/frontend-simple/index.html', 'r') as f:
    content = f.read()

# Add modal HTML before closing </div> of fusionTab container
modal_html = '''
...
# Find the closing of fusionTab and insert before it
# The structure is: <div id="fusionTab" ...> ...lots of content... </div>
# We need to close the script tags properly

# Find where to inject - right before the closing </script> of the main script
# We'll inject before the last </script> tag
parts = content.rsplit('</script>', 1)
if len(parts) == 2:
    content = parts[0] + modal_html + '</script>' + parts[1]

with open('/home/admin/.openclaw/workspace/crm-system/frontend-simple/index.html', 'w') as f:
    f.write(content)

print("Modal system installed successfully!")